...
graph TD
    ...
    FF -.funds.-> VANCE
    A16Z -.funds.-> VANCE
    YARVIN -.inspires.-> VANCE
    LAND -.inspires.-> YARVIN
    BALAJI -.inspires.-> SACKS
    VANCE & MUSK & SACKS & PCAST --> POL["Public policy: AI · crypto · defense"]
...
```mermaid
graph LR
    HAYEK["Hayek / Mises\n(market-as-discovery)"] --> EACC
    NIETZSCHE["Nietzsche\n(overman, elites)"] --> EACC & NRX
    RAND["Ayn Rand\n(heroic creator)"] --> EACC
    MARINETTI["Marinetti\n(futurism)"] --> EACC
    GIRARD["Girard / Strauss / Schmitt"] --> THIEL2["Thiel's thought"]
    THIEL2 --> NRX
    YARVINL["Yarvin / Land"] --> NRX["Neo-reaction"]
    SOV["The Sovereign Individual"] --> CRYPTO & NETSTATE
    CYPH["Cypherpunks"] --> CRYPTO["Crypto-anarchy"]
    CRYPTO --> NETSTATE["Network State"]
    MORE["Extropianism (More)"] --> TRANS["Transhumanism"]
    BOSTROM["Bostrom / Kurzweil"] --> TRANS
    TRANS --> EA["Effective Altruism / Longtermism"]
    TRANS --> EACC["e/acc"]
    EA -. opposes .-> EACC
    EACC --> SYNTH["RIGHT-WING SYNTHESIS 2025-26"]
    NRX --> SYNTH
    NETSTATE --> SYNTH
    EA -. internal adversary .-> SYNTH
```
