...
```mermaid
graph TD
    subgraph T["🔺 TOP — those who give the orders"]
        MUSK["Elon Musk"]
        THIEL["Peter Thiel"]
        AND["Marc Andreessen"]
    end
    subgraph R["⚙️ RELAYS — execution & State"]
        VANCE["JD Vance (VP)"]
        SACKS["David Sacks (AI/crypto)"]
        KARP["Alex Karp (Palantir)"]
    end
    subgraph I["🧠 IDEOLOGUES — supply the frame"]
        YARVIN["Curtis Yarvin"]
        LAND["Nick Land"]
        BALAJI["Balaji Srinivasan"]
    end
    subgraph O["🌫️ INTERNAL OPPOSITION — in decline"]
        EA["Bostrom / Yudkowsky (AI caution)"]
        HOFF["Reid Hoffman (dem. wing)"]
    end
    LAND --> YARVIN
    YARVIN --> THIEL & VANCE
    BALAJI --> SACKS
    ...
    MUSK --> VANCE
    T --> R
    R --> POWER["🏛️ Public policy: AI · crypto · defense · surveillance"]
    I -.mental frame.-> T
    O -.contests.-> T
```
